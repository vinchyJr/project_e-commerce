...
const Playstation: React.FC = () => {
  const [jeuxPlaystation, setJeuxPlaystation] = useState<Jeu[]>([]);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    const fetchGames = async () => {
      try {
        const response = await axios.get('http://localhost:3000/games?platform=playstation');
        ...
        setJeuxPlaystation(games);
      } catch (error) {
        setError('Aucun jeu dans Playsation');
  ...
  return (
    <div className="bg-color-dark">
      ...
      <div className="container mx-auto p-4 pt-32">
        <h2 className="text-2xl font-bold text-white mb-4 text-center">PlayStation Games</h2>
